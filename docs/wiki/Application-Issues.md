...
Ymir puts windows in the tiled state with [`prefer-no-csd`](./Configuration:-Miscellaneous.md#prefer-no-csd).
So if you hit this problem, comment out `prefer-no-csd` in the ymir config and restart WezTerm.
...
For some reason, DMABUF screencasts are disabled in the Zen Browser, so screencasting doesn't work out of the box on ymir.
...
Note that the ymir environment config does not propagate to apps and shells started by systemd, for example to DankMaterialShell and its application launcher.
You can set the variable in your login shell config (i.e. `~/.bash_profile`) instead, though keep in mind that then it will be set for all compositors, not just ymir.
...
This leads to ymir filling the transparent pixels inside the corners with black.
