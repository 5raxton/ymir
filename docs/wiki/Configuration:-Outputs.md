...
You can find all of these by running `ymir msg outputs`.
...
If the refresh rate is omitted, ymir will pick the highest refresh rate for the resolution.

If the mode is omitted altogether or doesn't work, ymir will try to pick one automatically.
...
Ymir is asking the monitor to run in a mode that is not supported by the manufacturer.
...
Ymir is asking the monitor to run in a mode not supported by the manufacturer.
Use at your own risk.
...
Ymir repositions outputs from scratch every time the output configuration changes (which includes monitors disconnecting and connecting).
...
You can check whether an output supports VRR in `ymir msg outputs`.
