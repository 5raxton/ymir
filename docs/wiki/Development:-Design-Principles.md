...
This is ymir state that is not immediately apparent from looking at the screen. This is not bad per se, but you should carefully consider how to reduce the surprise factor.
...
The [default config](https://github.com/ymir-wm/ymir/blob/main/resources/default-config.kdl) is intended to give a familiar, helpful, and not too jarring experience to new ymir users.
...
As such, new ymir users are expected to read through and tinker with the default ymir config.
...
Since <kbd>Alt</kbd> is a modifier in nested ymir, binds with explicit <kbd>Alt</kbd> are mainly the ones only useful on the host, for example spawning a screen locker.
