...
However, this means that ymir has to render the external monitor contents on the integrated GPU, then copy them over to the discrete GPU for display.
On some laptops this can cause lag and stuttering (it gets worse with monitor resolution and refresh rate).

If your laptop has a MUX switch—usually a GPU toggle in the UEFI settings—then you can switch it to use the discrete GPU, then ymir will render on the discrete GPU, and the external monitor won't lag.
Otherwise, you can try configuring ymir to render on the discrete GPU via the [`render-drm-device`](./Configuration:-Debug-Options.md#render-drm-device) debug option.
...
### Why doesn't ymir integrate Xwayland like other compositors?
...
- ymir isn't a Big Serious Desktop Environment which Must Support All Use Cases (and is Backed By Some Corporation).
...
No, but you can script the behavior you want with the [ymir IPC](./IPC.md).
...
This is ymir's locked session background.
...
They draw their own 1 px dark border around the window, which obscures one pixel of ymir's border.
