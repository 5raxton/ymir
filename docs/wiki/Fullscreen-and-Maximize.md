There are several ways to make a window big on ymir: maximizing the column, maximizing the window to edges, and fullscreening the window.
...
Ymir renders a solid black backdrop behind fullscreen windows.
...
The best opportunity for this is during the *initial configure* sequence when the window tells ymir everything it should know before opening the window.
...
However, some clients tend to request to be maximized shortly *after* the initial configure sequence, when the ymir already sent them the initial size request (sometimes even after showing on screen, resulting in a quick resize right after opening).
From ymir's point of view, the window is already open by this point, so if the window does this, then the `open-maximized-to-edges` and `open-fullscreen` window rules don't do anything.
...
When in windowed fullscreen, you can use the ymir action to maximize or unmaximize the window.
