...
Presently, there is a quirk in the NVIDIA drivers that affects ymir's VRAM usage (the driver does not properly release VRAM back into the pool). Ymir *should* use on the order of 100 MiB of VRAM (as checked in [nvtop](https://github.com/Syllo/nvtop)); if you see anywhere close to 1 GiB of VRAM in use, you are likely hitting this issue (heap not returning freed buffers to the driver).
...
* write the following JSON blob to set the `GLVidHeapReuseRatio` config value for the `ymir` process into the file `/etc/nvidia/nvidia-application-profiles-rc.d/50-limit-free-buffer-pool-in-wayland-compositors.json`:
...
This debug flag has since been removed because the problem was properly fixed in ymir.
