...
To check that the integration works, verify that the ymir output says something like `listening on X11 socket: :0`:
...
ymir[2474]: 2025-08-29T04:07:40.700137Z  INFO ymir: listening on X11 socket: :0
systemd[2338]: Started ymir.service - A scrollable-tiling Wayland compositor.
$ echo $DISPLAY
:0
```
...
We're using xwayland-satellite rather than Xwayland directly because [X11 is very cursed](./FAQ.md#why-doesnt-ymir-integrate-xwayland-like-other-compositors).
xwayland-satellite takes on the bulk of the work dealing with the X11 peculiarities from us, giving ymir normal Wayland windows to manage.
...
1. Run it inside ymir with the `labwc` command.
...
- `env DISPLAY=:0 xsel -ob | wl-copy` to copy from Xwayland to ymir clipboard
- `wl-paste -n | env DISPLAY=:0 xsel -ib` to copy from ymir to Xwayland clipboard
...
2. Run `cage -- /path/to/application` and enjoy your X11 program on ymir.
