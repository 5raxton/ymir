...
use crate::ymir::{Ymir, RedrawState};
...
impl Headless {
    ...
    pub fn init(&mut self, _ymir: &mut Ymir) {}
    ...
    pub fn add_output(&mut self, ymir: &mut Ymir, n: u8, size: (u16, u16)) {
        let connector = format!("headless-{n}");
        let make = "ymir".to_string();
        ...
        ymir.add_output(output, None, false);
    }
    ...
    pub fn render(&mut self, ymir: &mut Ymir, output: &Output) -> RenderResult {
        ...
        let mut presentation_feedbacks = ymir.take_presentation_feedbacks(output, &states);
