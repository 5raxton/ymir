...
use libc::dev_t;
use ymir_config::output::{MaxBpc, Modeline};
use ymir_config::{Config, OutputName};
use ymir_ipc::{HSyncPolarity, VSyncPolarity};
...
use crate::ymir::{Ymir, RedrawState, State};
...
impl OutputDevice {
    ...
    fn cleanup_mismatching_resources(
        ...
        for (conn, info) in self.drm_scanner.connectors() {
            // We only keep the connector if it has a CRTC and the output isn't off in ymir.
...
impl Tty {
    pub fn new(
        ...
        let (session, notifier) = LibSeatSession::new().context(
            "Error creating a session. This might mean that you're trying to run ymir on a TTY \
        ...
        let udev_dispatcher = Dispatcher::new(udev_backend, move |event, _, state: &mut State| {
            state.backend.tty().on_udev_event(&mut state.ymir, event);
        });
        event_loop
        ...
        // If the session is not active at startup (e.g. ymir was launched from a different TTY),
        ...
        event_loop
            .insert_source(notifier, move |event, _, state| {
                state.backend.tty().on_session_event(&mut state.ymir, event);
            })
            .unwrap();
    ...
    pub fn init(&mut self, ymir: &mut Ymir) {
        ...
        //
        // This can happen when starting ymir while having a different TTY active (e.g. via tmux).
        ...
        {
            if let Err(err) = self.device_added(primary_device_id, primary_device_path, ymir) {
                warn!(
        ...
        for (device_id, path) in udev.device_list() {
            ...
            if let Err(err) = self.device_added(device_id, path, ymir) {
                warn!("error adding device: {err:?}");
            }
        }
    }

    fn on_udev_event(&mut self, ymir: &mut Ymir, event: UdevEvent) {
        ...
        match event {
            ...
            UdevEvent::Changed { device_id } => {
                ...
                self.device_changed(device_id, ymir, false)
            }
            UdevEvent::Removed { device_id } => {
                ...
                self.device_removed(device_id, ymir)
            }
        }
    }

    fn on_session_event(&mut self, ymir: &mut Ymir, event: SessionEvent) {
        ...
        match event {
            ...
            SessionEvent::ActivateSession => {
                ...
                for node in removed_devices {
                    ...
                    self.device_removed(node.dev_id(), ymir);
                }
                ...
                for node in remained_devices {
                    ...
                    self.device_changed(node.dev_id(), ymir, true);
                ...
                if self.update_output_config_on_resume {
                    self.on_output_config_changed(ymir);
                }

                self.refresh_ipc_outputs(ymir);

                ymir.notify_activity();
                ymir.monitors_active = true;
                self.set_monitors_active(true);
                ymir.queue_redraw_all();
            }
        }
    }
    ...
    fn device_added(
        ...
        if render_node == Some(self.primary_render_node) && self.dmabuf_global.is_none() {
            ...
            if let Err(err) = renderer.bind_wl_display(&ymir.display_handle) {
            ...
            .insert_source(drm_notifier, move |event, meta, state| {
                ...
                match event {
                    DrmEvent::VBlank(crtc) => {
                        ...
                        tty.on_vblank(&mut state.ymir, node, crtc, meta);
                    }
        ...
        let drm_lease_state = DrmLeaseState::new::<State>(&ymir.display_handle, &node)
    ...
    fn device_changed(&mut self, device_id: dev_t, ymir: &mut Ymir, cleanup: bool) {
        ...
        for crtc in &removed {
            self.connector_disconnected(ymir, node, *crtc);
        }
        ...
        if cleanup {
            ...
            let disable_laptop_panels = self.should_disable_laptop_panels(ymir.is_lid_closed);
    ...
    fn device_removed(&mut self, device_id: dev_t, ymir: &mut Ymir) {
        ...
        if let Some(render_node) = device.render_node {
            ...
            if was_last && render_node == self.primary_render_node {
                ...
                if let Some(global) = self.dmabuf_global.take() {
                    ymir.dmabuf_state
                        .disable_global::<State>(&ymir.display_handle, &global);
                    ymir.event_loop
                        .insert_source(
                            ...
                            move |_, _, state| {
                                state
                                    .ymir
                                    .dmabuf_state
                                    .destroy_global::<State>(&state.ymir.display_handle, global);
                                TimeoutAction::Drop
                            },
                        )
                        .unwrap();
        ...
        ymir.event_loop.remove(device.token);
    ...
    fn connector_connected(
        ...
        ymir.add_output(output.clone(), Some(refresh_interval(mode)), vrr_enabled);

        if ymir.monitors_active {
            // Redraw the new monitor.
            ymir.event_loop.insert_idle(move |state| {
                // Guard against output disconnecting before the idle has a chance to run.
                if state.ymir.output_state.contains_key(&output) {
                    state.ymir.queue_redraw(&output);
                }
            });
        }
    ...
    fn connector_disconnected(&mut self, ymir: &mut Ymir, node: DrmNode, crtc: crtc::Handle) {
        let Some(device) = self.devices.get_mut(&node) else {
    ...
    fn on_vblank(
        ...
        let presentation_time = if ymir.config.borrow().debug.emulate_zero_presentation_time {
            Duration::ZERO
        } else {
            presentation_time
        };
        ...
        let Some(output_state) = ymir.output_state.get_mut(&output) else {
        ...
        if redraw_needed || output_state.unfinished_animations_remain {
            ...
            ymir.queue_redraw(&output);
        } else {
            ymir.send_frame_callbacks(&output);
        }
    }

    fn on_estimated_vblank_timer(&self, ymir: &mut Ymir, output: Output) {
    ...
    pub fn render(
        ...
        let mut elements = ymir.render_to_vec(ctx, output, true);

        // Visualize the damage, if enabled.
        if ymir.debug_draw_damage {
            let output_state = ymir.output_state.get_mut(output).unwrap();
        ...
        let flags = {
            ...
            if debug.skip_cursor_only_updates_during_vrr {
                let output_state = ymir.output_state.get(output).unwrap();
                if output_state.frame_clock.vrr() {
        ...
        match drm_compositor.render_frame::<_, _>(&mut renderer, &elements, [0.; 4], flags) {
            Ok(res) => {
                ...
                ymir.update_primary_scanout_output(output, &res.states);
                if let Some(dmabuf_feedback) = surface.dmabuf_feedback.as_ref() {
                    ymir.send_dmabuf_feedbacks(output, dmabuf_feedback, &res.states);
                }

                if !res.is_empty {
                    let presentation_feedbacks =
                        ymir.take_presentation_feedbacks(output, &res.states);
        ...
        queue_estimated_vblank_timer(ymir, output.clone(), target_presentation_time);
    ...
    fn refresh_ipc_outputs(&self, ymir: &mut Ymir) {
        ...
        for (node, device) in &self.devices {
            for (connector, crtc) in device.drm_scanner.crtcs() {
                ...
                let mut modes: Vec<ymir_ipc::Mode> = connector
                    .modes()
                    .iter()
                ...
                let ipc_output = ymir_ipc::Output {
                    name: connector_name,
    ...
    pub fn on_output_config_changed(&mut self, ymir: &mut Ymir) {
        ...
        for (node, connector, crtc, _name) in to_connect {
            if let Err(err) = self.connector_connected(ymir, node, connector, crtc) {
...
fn queue_estimated_vblank_timer(
    ...
    let token = ymir
        ...
        .insert_source(timer, move |_, _, data| {
            data.backend
                .tty()
                .on_estimated_vblank_timer(&mut data.ymir, output.clone());
            TimeoutAction::Drop
        })
        .unwrap();
    output_state.redraw_state = RedrawState::WaitingForEstimatedVBlank(token);
...
fn pick_mode(
    connector: &connector::Info,
    target: Option<ymir_config::output::Mode>,
...
mod tests {
    ...
    use ymir_config::output::Modeline;
    use ymir_ipc::{HSyncPolarity, VSyncPolarity};
