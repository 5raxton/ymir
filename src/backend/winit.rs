...
impl Winit {
    pub fn new(
        ...
        let builder = WindowAttributes::default()
            ...
            .with_platform_attributes(Box::new(
                WindowAttributesWayland::default().with_name("ymir", ""),
            ));
            ...
            .insert_source(winit, move |event, _, state| match event {
                WinitEvent::Resized { size, .. } => {
...

                    {
                        ...
                        state.ymir.ipc_outputs_changed = true;
                    }

                    state.ymir.output_resized(&winit.output);
                }
                WinitEvent::Input(event) => state.process_input_event(event),
                WinitEvent::Focus(_) => (),
                WinitEvent::Redraw => state.ymir.queue_redraw(&state.backend.winit().output),
                WinitEvent::CloseRequested => state.ymir.stop_signal.stop(),
            })
            .unwrap();
    ...
    pub fn init(&mut self, ymir: &mut Ymir) {
        ...
        self.create_dmabuf_global(ymir);

        ymir.add_output(self.output.clone(), None, false);
    }

    pub fn create_dmabuf_global(&mut self, ymir: &mut Ymir) {
        let renderer = self.backend.renderer();
        ...
        let dmabuf_global = match default_feedback() {
            Ok(feedback) => ymir
                .dmabuf_state
                .create_global_with_default_feedback::<State>(&ymir.display_handle, &feedback),
            Err(err) => {
                debug!("failed building default dmabuf feedback, falling back to v3: {err:?}");
                let primary_formats = renderer.dmabuf_formats();
                ymir.dmabuf_state
                    .create_global::<State>(&ymir.display_handle, primary_formats)
            }
        };
    ...
    pub fn render(&mut self, ymir: &mut Ymir, output: &Output) -> RenderResult {
        ...
        if let Some(damage) = res.damage {
            ...
            let mut presentation_feedbacks = ymir.take_presentation_feedbacks(output, &res.states);
