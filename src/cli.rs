...
use ymir_ipc::{Action, OutputAction};
...
pub struct Cli {
    /// Path to config file (default: `$XDG_CONFIG_HOME/ymir/init.lua`).
    ///
...
#[derive(Subcommand)]
pub enum Sub {
    /// Communicate with the running ymir instance.
    Msg {
        #[command(subcommand)]
        msg: Msg,
...
pub enum Msg {
    ...
    Output {
        /// Output name.
        ///
        /// Run `ymir msg outputs` to see the output names.
        #[arg()]
        output: String,
    ...
    EventStream,
    /// Print the version of the running ymir instance.
    Version,
    /// Request an error from the running ymir instance.
    RequestError,
