...
use crate::ymir::{PointContents, State};
...
pub enum A11yManagerToYmir {
    QueryPointer,
}

pub enum YmirToA11yManager {
...
pub struct PointerLocator {
    ...
    iface: Arc<OnceLock<InterfaceRef<Self>>>,
    to_ymir: calloop::channel::Sender<A11yManagerToYmir>,
    from_ymir: async_channel::Receiver<YmirToA11yManager>,
}
...
impl PointerLocator {
    ...
    async fn query_pointer(
        ...
        if let Err(err) = self.to_ymir.send(A11yManagerToYmir::QueryPointer) {
            warn!("error sending message to ymir: {err:?}");
            return Err(A11yError::Failed("internal error".to_owned()));
        }

        let rv = match self.from_ymir.recv().await {
            Ok(YmirToA11yManager::PointerContents(Some((data, x, y)))) => Ok((data, x, y)),
            Ok(YmirToA11yManager::PointerContents(None)) => Err(A11yError::UnknownToplevel),
            Err(err) => {
...
impl State {
    pub fn a11y_process_key(
        ...
        if self.ymir.a11y_manager.is_none() {
            return KbMonBlock::Pass;
        }
        ...
        let Some(manager) = &self.ymir.a11y_manager else {
            return KbMonBlock::Pass;
        };
    ...
    pub fn on_a11y_manager_msg(
        &mut self,
        to_a11y: &async_channel::Sender<YmirToA11yManager>,
        msg: A11yManagerToYmir,
    ) {
        let A11yManagerToYmir::QueryPointer = msg;
        ...
        let contents = match &self.ymir.pointer_contents {
            PointContents {
            ...
            _ => {
                // The focused client is ymir itself.
                Some((PointerAppData::default(), pointer_pos.x, pointer_pos.y))
            }
        };

        let msg = YmirToA11yManager::PointerContents(contents);
