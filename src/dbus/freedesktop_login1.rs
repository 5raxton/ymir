...
pub enum Login1ToYmir {
    LidClosedChanged(bool),
}

pub fn start(
    to_ymir: calloop::channel::Sender<Login1ToYmir>,
    ...
    let future = async move {
        ...
        if let Err(err) = to_ymir.send(Login1ToYmir::LidClosedChanged(lid_closed)) {
            warn!("error sending initial lid state to ymir: {err:?}");
            return;
        };
