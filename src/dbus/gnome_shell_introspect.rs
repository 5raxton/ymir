...
pub struct Introspect {
    to_ymir: calloop::channel::Sender<IntrospectToYmir>,
    from_ymir: async_channel::Receiver<YmirToIntrospect>,
}

pub enum IntrospectToYmir {
    GetWindows,
}

pub enum YmirToIntrospect {
...
impl Introspect {
    async fn get_windows(&self) -> fdo::Result<HashMap<u64, WindowProperties>> {
        if let Err(err) = self.to_ymir.send(IntrospectToYmir::GetWindows) {
            warn!("error sending message to ymir: {err:?}");
            return Err(fdo::Error::Failed("internal error".to_owned()));
        }

        match self.from_ymir.recv().await {
            Ok(YmirToIntrospect::Windows(windows)) => Ok(windows),
            Err(err) => {
