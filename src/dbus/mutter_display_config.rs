...
pub struct DisplayConfig {
    to_ymir: calloop::channel::Sender<HashMap<String, Option<ymir_config::Output>>>,
        ...
        for output in self.ipc_outputs.lock().unwrap().values() {
            ...
            if let Some(logical) = output.logical.as_ref() {
                let transform = match logical.transform {
                    ymir_ipc::Transform::Normal => 0,
                    ymir_ipc::Transform::_90 => 1,
                    ymir_ipc::Transform::_180 => 2,
                    ymir_ipc::Transform::_270 => 3,
                    ymir_ipc::Transform::Flipped => 4,
                    ymir_ipc::Transform::Flipped90 => 5,
                    ymir_ipc::Transform::Flipped180 => 6,
                    ymir_ipc::Transform::Flipped270 => 7,
                };
    ...
    async fn apply_monitors_config(
        ...
        for requested_config in logical_monitor_configs {
            ...
            for (connector, mode, _props) in requested_config.monitors {
                ...
                new_conf.insert(
                    connector.clone(),
                    Some(ymir_config::Output {
                        off: false,
                        name: connector,
                        scale: Some(ymir_config::FloatOrInt(requested_config.scale)),
                        transform: match requested_config.transform {
                            0 => ymir_ipc::Transform::Normal,
                            1 => ymir_ipc::Transform::_90,
                            2 => ymir_ipc::Transform::_180,
                            3 => ymir_ipc::Transform::_270,
                            4 => ymir_ipc::Transform::Flipped,
                            5 => ymir_ipc::Transform::Flipped90,
                            6 => ymir_ipc::Transform::Flipped180,
                            7 => ymir_ipc::Transform::Flipped270,
                            x => {
                                return Err(zbus::fdo::Error::Failed(format!(
                                    "Unknown transform {x}",
                                )))
                        ...
                        position: Some(ymir_config::Position {
                            x: requested_config.x,
                            y: requested_config.y,
                        }),
                        mode: Some(ymir_config::output::Mode {
                            custom: false,
                            mode: ymir_ipc::ConfiguredMode::from_str(&mode).map_err(|e| {
        ...
        if let Err(err) = self.to_ymir.send(new_conf) {
...
// Adapted from Mutter.
fn make_display_name(output: &ymir_ipc::Output, is_laptop_panel: bool) -> String {
    if is_laptop_panel {
