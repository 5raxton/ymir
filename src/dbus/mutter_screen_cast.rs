...
pub struct ScreenCast {
    ...
    to_ymir: calloop::channel::Sender<ScreenCastToYmir>,
...
impl Session {
    ...
    pub async fn stop(
        ...
        if let Err(err) = self.to_ymir.send(ScreenCastToYmir::StopCast {
            ...
            warn!("error sending StopCast to ymir: {err:?}");
        }
...
impl Drop for Session {
    fn drop(&mut self) {
        let _ = self.to_ymir.send(ScreenCastToYmir::StopCast {
            session_id: self.id,
        });
    }
}
...
impl Stream {
    ...
    fn start(&self, ctxt: SignalEmitter<'static>) {
        ...
        let msg = ScreenCastToYmir::StartCast {
        ...
        if let Err(err) = self.to_ymir.send(msg) {
            warn!("error sending StartCast to ymir: {err:?}");
        }
    }
}
