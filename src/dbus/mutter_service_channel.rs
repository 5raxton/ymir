...
use super::Start;
use crate::ymir::NewClient;

pub struct ServiceChannel {
    to_ymir: calloop::channel::Sender<NewClient>,
}
...
impl ServiceChannel {
    async fn open_wayland_service_connection(
        ...
        if let Err(err) = self.to_ymir.send(client) {
...
impl ServiceChannel {
    pub fn new(to_ymir: calloop::channel::Sender<NewClient>) -> Self {
        Self { to_ymir }
    }
}
