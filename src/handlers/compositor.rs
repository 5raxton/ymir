...
use crate::ymir::{CastTarget, ClientState, LockState, State};
...
impl CompositorHandler for State {
    fn compositor_state(&mut self) -> &mut CompositorState {
        &mut self.ymir.compositor_state
    }
    ...
    fn new_subsurface(&mut self, surface: &WlSurface, parent: &WlSurface) {
        ...
        if let Some(output) = self.ymir.output_for_root(&root) {
    ...
    fn commit(&mut self, surface: &WlSurface) {
        ...
        if surface == &root_surface {
            ...
            if let Entry::Occupied(entry) = self.ymir.unmapped_windows.entry(surface.clone()) {
                if is_mapped(surface) {
                    ...
                    } = state
                    {
                        ...
                        let output =
                            output.filter(|o| self.ymir.layout.monitor_for_output(o).is_some());

                        // Check that the workspace still exists.
                        let workspace_id = workspace_name
                            .as_deref()
                            .and_then(|n| self.ymir.layout.find_workspace_by_name(n))
                            .map(|(_, ws)| ws.id());
                    ...
                    if let Some((mapped, _)) = self.ymir.layout.find_window_and_output(surface) {
                    ...
                    if let Some(output) = output {
                        self.ymir.layout.start_open_animation_for_window(&window);

                        let new_focus = self.ymir.layout.focus().map(|m| &m.window);
            ...
            if let Some((mapped, output)) = self.ymir.layout.find_window_and_output(surface) {
                let window = mapped.window.clone();
                let output = output.cloned();
                ...
                if !is_mapped {
                    ...
                    self.ymir.unmapped_windows.insert(surface.clone(), unmapped);
                ...
                self.ymir.window_mru_ui.update_window(&self.ymir.layout, id);
                self.ymir.layout.update_window(&window, serial);

                // Move the toplevel according to the attach offset.
                if let Some(delta) = buffer_delta {
                    if delta.x != 0 || delta.y != 0 {
                        let (x, y) = delta.to_f64().into();
                        self.ymir.layout.move_floating_window(
                            Some(&window),
        ...
        let root_window_output = self.ymir.layout.find_window_and_output(&root_surface);
        if let Some((mapped, output)) = root_window_output {
            ...
            self.ymir
                .window_mru_ui
                .update_window(&self.ymir.layout, mapped.id());
        ...
        if let Some(popup) = self.ymir.popups.find_popup(&root_surface) {
        ...
        if matches!(
            &self.ymir.cursor_manager.cursor_image(),
        ...
        if matches!(&self.ymir.dnd_icon, Some(icon) if icon.surface == root_surface) {
            let dnd_icon = self.ymir.dnd_icon.as_mut().unwrap();
        ...
        for (output, state) in &self.ymir.output_state {
            if let Some(lock_surface) = &state.lock_surface {
                if lock_surface.wl_surface() == &root_surface {
                    if matches!(self.ymir.lock_state, LockState::WaitingForSurfaces { .. }) {
                        self.ymir.maybe_continue_to_locking();
                    } else {
                        self.ymir.queue_redraw(&output.clone());
                    }
    ...
    fn destroyed(&mut self, surface: &WlSurface) {
        ...
        if let Some(root) = self.ymir.root_surface.get(surface) {
            if let Some((mapped, output)) = self.ymir.layout.find_window_and_output(root) {
                let window = mapped.window.clone();
                let output = output.cloned();
        ...
        if let Some(hook) = self.ymir.dmabuf_pre_commit_hook.remove(surface) {
...
impl State {
    pub fn add_default_dmabuf_pre_commit_hook(&mut self, surface: &WlSurface) {
        ...
        let s = surface.clone();
        if let Some(prev) = self.ymir.dmabuf_pre_commit_hook.insert(s, hook) {
