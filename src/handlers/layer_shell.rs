...
impl WlrLayerShellHandler for State {
    fn shell_state(&mut self) -> &mut WlrLayerShellState {
        &mut self.ymir.layer_shell_state
    }
    ...
    fn new_layer_surface(
        ...
        let output = if let Some(wl_output) = &wl_output {
            self.ymir.output_from_resource(wl_output)
        } else {
            self.ymir.layout.active_output().cloned()
        };
    ...
    fn layer_destroyed(&mut self, surface: WlrLayerSurface) {
        ...
        self.ymir.unmapped_layer_surfaces.remove(wl_surface);

        let output = if let Some((output, mut map, layer)) =
            self.ymir.layout.outputs().find_map(|o| {
            ...
            self.ymir.mapped_layer_surfaces.remove(&layer);
            Some(output)
        } else {
            None
        };
...
impl State {
    pub fn layer_shell_handle_commit(&mut self, surface: &WlSurface) -> bool {
        ...
        if is_mapped(surface) {
            let was_unmapped = self.ymir.unmapped_layer_surfaces.remove(surface);
            ...
            if was_unmapped {
                ...
                let rules = ResolvedLayerRules::compute(rules, layer, self.ymir.is_at_startup);
                ...
                if let Some(mapped) = self.ymir.mapped_layer_surfaces.get_mut(layer) {
                    // Check if the layer changed.
                    if mapped.take_recompute_rules_on_commit() {
                        let config = self.ymir.config.borrow();
                        if mapped
                            .recompute_layer_rules(&config.layer_rules, self.ymir.is_at_startup)
                        {
            ...
            //
            // https://lab.braxton.onl/braxton/ymir/issues/641
            ...
            if was_unmapped && on_demand {
                ...
                // little annoying.
                self.ymir.layer_shell_on_demand_focus = Some(layer.clone());
            }
        } else {
            // The surface is unmapped.
            if self.ymir.mapped_layer_surfaces.remove(layer).is_some() {
                // A mapped surface got unmapped via a null commit. Now it needs to do a new
                // initial commit again.
                self.ymir.unmapped_layer_surfaces.insert(surface.clone());
            } else {
...
fn add_mapped_layer_pre_commit_hook(layer: &LayerSurface) -> HookId {
    add_pre_commit_hook::<State, _>(layer.wl_surface(), move |state, _dh, surface| {
        ...
        if layer_changed {
            for mapped in state.ymir.mapped_layer_surfaces.values_mut() {
