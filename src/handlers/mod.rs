...
use crate::ymir::{DndIcon, NewClient, State};
...
impl SeatHandler for State {
    ...
    fn cursor_image(&mut self, _seat: &Seat<Self>, mut image: CursorImageStatus) {
        ...
        self.ymir.cursor_manager.set_cursor_image(image);
        // FIXME: more granular
        self.ymir.queue_redraw_all();
    }

    fn focus_changed(&mut self, seat: &Seat<Self>, focused: Option<&WlSurface>) {
        let dh = &self.ymir.display_handle;
...
impl PointerConstraintsHandler for State {
    ...
    fn cursor_position_hint(
        ...
        let Some((ref surface_under_pointer, origin)) = self.ymir.pointer_contents.surface else {
            return;
        };
        ...
        let target = self
            ...
            .and_then(|output| self.ymir.global_space.output_geometry(output))
        ...
        self.ymir.pointer_constraint_position_hint = Some(target);
    }
    ...
    fn remove_constraint(
        ...
        let Some(target) = self.ymir.pointer_constraint_position_hint.take() else {
...
impl KeyboardShortcutsInhibitHandler for State {
    fn keyboard_shortcuts_inhibit_state(&mut self) -> &mut KeyboardShortcutsInhibitState {
        &mut self.ymir.keyboard_shortcuts_inhibit_state
    }
...
impl DataDeviceHandler for State {
    fn data_device_state(&mut self) -> &mut DataDeviceState {
        &mut self.ymir.data_device_state
    }
}
...
impl WaylandDndGrabHandler for State {
    fn dnd_requested<S: dnd::Source>(
        ...
        self.ymir.dnd_icon = icon.map(|surface| DndIcon {
            surface,
        ...
        match type_ {
            dnd::GrabType::Pointer => {
                ...
                let grab =
                    DnDGrab::new_pointer(&self.ymir.display_handle, start_data, source, seat);
                pointer.set_grab(self, grab, serial, Focus::Keep);
            }
            dnd::GrabType::Touch => {
                let touch = seat.get_touch().unwrap();
                let start_data = touch.grab_start_data().unwrap();
                let grab = DnDGrab::new_touch(&self.ymir.display_handle, start_data, source, seat);
...
impl DndGrabHandler for State {
    fn dropped(
        ...
        if let Some(target) = validated.then_some(target).flatten() {
            let root = self.ymir.find_root_shell_surface(target);
...
impl SessionLockHandler for State {
    fn lock_state(&mut self) -> &mut SessionLockManagerState {
        &mut self.ymir.session_lock_state
    }

    fn lock(&mut self, confirmation: SessionLocker) {
        self.ymir.lock(confirmation);
    }
    ...
    fn new_surface(&mut self, surface: LockSurface, output: WlOutput) {
        let Some(output) = self.ymir.output_from_resource(&output) else {
            warn!("no Output matching WlOutput");
            return;
        };

        configure_lock_surface(&surface, &output);
        self.ymir.new_lock_surface(surface, &output);
    }
}
...
impl IdleNotifierHandler for State {
    fn idle_notifier_state(&mut self) -> &mut IdleNotifierState<Self> {
        &mut self.ymir.idle_notifier_state
    }
}

impl IdleInhibitHandler for State {
    fn inhibit(&mut self, surface: WlSurface) {
        self.ymir.idle_inhibiting_surfaces.insert(surface);
    }

    fn uninhibit(&mut self, surface: WlSurface) {
        self.ymir.idle_inhibiting_surfaces.remove(&surface);
    }
}

impl ForeignToplevelHandler for State {
    fn foreign_toplevel_manager_state(&mut self) -> &mut ForeignToplevelManagerState {
        &mut self.ymir.foreign_toplevel_state
    }

    fn activate(&mut self, wl_surface: WlSurface) {
        if let Some((mapped, _)) = self.ymir.layout.find_window_and_output(&wl_surface) {
            let window = mapped.window.clone();
            self.ymir.layout.activate_window(&window);
            self.ymir.layer_shell_on_demand_focus = None;
            self.ymir.queue_redraw_all();
    ...
    fn set_fullscreen(&mut self, wl_surface: WlSurface, wl_output: Option<WlOutput>) {
        if let Some((mapped, current_output)) = self.ymir.layout.find_window_and_output(&wl_surface)
        {
            let window = mapped.window.clone();

            if let Some(requested_output) =
                wl_output.and_then(|o| self.ymir.output_from_resource(&o))
            {
...
impl ExtWorkspaceHandler for State {
    fn ext_workspace_manager_state(&mut self) -> &mut ExtWorkspaceManagerState {
        &mut self.ymir.ext_workspace_state
    }

    fn activate_workspace(&mut self, id: WorkspaceId) {
        let reference = ymir_config::WorkspaceReference::Id(id.get());
        if let Some((mut output, index)) = self.ymir.find_output_and_workspace_index(reference) {
    ...
    fn assign_workspace(&mut self, ws_id: WorkspaceId, output: Output) {
        let reference = ymir_config::WorkspaceReference::Id(ws_id.get());
        if let Some((old_output, old_idx)) = self.ymir.find_output_and_workspace_index(reference) {
            self.ymir
                .layout
...
impl ScreencopyHandler for State {
    fn frame(&mut self, manager: &ZwlrScreencopyManagerV1, screencopy: Screencopy) {
        ...
        if !self.ymir.output_exists(screencopy.output()) {
        ...
        if screencopy.with_damage() {
            self.ymir.screencopy_state.push(manager, screencopy);
        } else {
...
impl GammaControlHandler for State {
    fn gamma_control_manager_state(&mut self) -> &mut GammaControlManagerState {
        &mut self.ymir.gamma_control_manager_state
    }
...
impl XdgActivationHandler for State {
    fn activation_state(&mut self) -> &mut XdgActivationState {
        &mut self.ymir.activation_state
    }
        ...
        if token_data.timestamp.elapsed() < XDG_ACTIVATION_TOKEN_TIMEOUT {
            if let Some((mapped, _)) = self.ymir.layout.find_window_and_output_mut(&surface) {
                ...
                match mapped.rules().on_xdg_activate {
                    Some(ymir_config::OnXdgActivate::Ignore) => {}
                    Some(ymir_config::OnXdgActivate::SetUrgent) => {
                        mapped.set_urgent(true);
                        self.ymir.queue_redraw_all();
                    }
                    Some(ymir_config::OnXdgActivate::Focus) => {
                        self.ymir.layout.activate_window(&window);
                        self.ymir.layer_shell_on_demand_focus = None;
                        self.ymir.queue_redraw_all();
                    }
            ...
            } else if let Some(unmapped) = self.ymir.unmapped_windows.get_mut(&surface) {
                unmapped.activation_token_data = Some(token_data);
            }
        }

        self.ymir.activation_state.remove_token(&token);
    }
}
...
impl OutputManagementHandler for State {
    fn output_management_state(&mut self) -> &mut OutputManagementManagerState {
        &mut self.ymir.output_management_state
    }

    fn apply_output_config(&mut self, config: ymir_config::Outputs) {
        self.ymir.config.borrow_mut().outputs = config;
