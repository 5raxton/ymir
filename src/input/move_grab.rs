...
impl MoveGrab {
    pub fn new(
        ...
        let (output, pos_within_output) = state.ymir.output_under(location)?;
    ...
    fn on_ungrab(&mut self, data: &mut State) {
        let layout = &mut data.ymir.layout;
        match self.gesture {
    ...
    fn begin_move(&mut self, data: &mut State) -> bool {
        if !data.ymir.layout.interactive_move_begin(
            self.window.clone(),
            &self.start_output,
    ...
    fn on_frame(&mut self, data: &mut State) -> bool {
        ...
        match self.gesture {
            ...
            GestureState::Move => {
                ...
                let ongoing = data.ymir.layout.interactive_move_update(
                    &self.window,
                    delta,
                    output,
                    pos_within_output,
            ...
            GestureState::ViewOffset => {
                let res = data.ymir.layout.view_offset_gesture_update(
                    -relative_delta.x,
                    timestamp,
                    false,
                );
    ...
    fn on_toggle_floating(&mut self, data: &mut State) -> bool {
        ...
        data.ymir.layout.toggle_window_floating(Some(&self.window));
        data.ymir.queue_redraw_all();
