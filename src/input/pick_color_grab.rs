...
impl PickColorGrab {
    ...
    fn on_ungrab(&mut self, state: &mut State) {
        if let Some(tx) = state.ymir.pick_color.take() {
    ...
    fn pick_color_at_point(location: Point<f64, Logical>, data: &mut State) -> Option<PickedColor> {
        let (output, pos_within_output) = data.ymir.output_under(location)?;
        let output = output.clone();

        data.backend
            .with_primary_renderer(|renderer| {
                data.ymir.update_render_elements(Some(&output));
                ...
                let elements = data.ymir.render_to_vec(ctx, &output, false);
...
impl PointerGrab<State> for PickColorGrab {
    ...
    fn button(
        ...
        data.ymir.suppressed_buttons.insert(event.button);

        if let Some(tx) = data.ymir.pick_color.take() {
