...
use ymir_config::OutputName;
use ymir_ipc::state::{EventStreamState, EventStreamStatePart as _};
use ymir_ipc::{
...
impl IpcServer {
    pub fn start(
        ...
        let socket_path = if let Some(wayland_socket_name) = wayland_socket_name {
            ...
            let socket_name = format!("ymir.{wayland_socket_name}.{}.sock", process::id());
...
fn on_new_ipc_client(state: &mut State, stream: UnixStream) {
    ...
    let stream = match state.ymir.event_loop.adapt_io(stream) {
        Ok(stream) => stream,
        Err(err) => {
    ...
    let ipc_server = state.ymir.ipc_server.as_ref().unwrap();

    let ctx = ClientCtx {
        event_loop: state.ymir.event_loop.clone(),
        scheduler: state.ymir.scheduler.clone(),
    ...
    if let Err(err) = state.ymir.scheduler.schedule(future) {
    ...
    let response = match request {
        ...
        Request::Layers => {
            ...
            ctx.event_loop.insert_idle(move |state| {
                ...
                for output in state.ymir.global_space.outputs() {
                    let name = output.name();
                    for surface in layer_map_for_output(output).layers() {
                        let layer = match surface.layer() {
                            Layer::Background => ymir_ipc::Layer::Background,
                            Layer::Bottom => ymir_ipc::Layer::Bottom,
                            Layer::Top => ymir_ipc::Layer::Top,
                            Layer::Overlay => ymir_ipc::Layer::Overlay,
                        };
                        let keyboard_interactivity =
                            match surface.cached_state().keyboard_interactivity {
                                KeyboardInteractivity::None => {
                                    ymir_ipc::LayerSurfaceKeyboardInteractivity::None
                                }
                                KeyboardInteractivity::Exclusive => {
                                    ymir_ipc::LayerSurfaceKeyboardInteractivity::Exclusive
                                }
                                KeyboardInteractivity::OnDemand => {
                                    ymir_ipc::LayerSurfaceKeyboardInteractivity::OnDemand
                                }
                            };

                        layers.push(ymir_ipc::LayerSurface {
        ...
        Request::PickWindow => {
            let (tx, rx) = async_channel::bounded(1);
            ctx.event_loop.insert_idle(move |state| {
                let pointer = state.ymir.seat.get_pointer().unwrap();
                ...
                state.ymir.pick_window = Some(tx);
                state
                    .ymir
                    .cursor_manager
...
fn validate_action(action: &Action) -> Result<(), String> {
    ...
    | Action::LoadConfigFile { path } = action
    {
        if let Some(path) = path {
            // Relative paths are resolved against the ymir compositor's working directory, which
...
fn make_ipc_window(
    ...
    with_toplevel_role(mapped.toplevel(), |role| ymir_ipc::Window {
        id: mapped.id().get(),
...
impl State {
    pub fn ipc_keyboard_layouts_changed(&mut self) {
        ...
        let Some(server) = &self.ymir.ipc_server else {
            return;
        };
    ...
    pub fn ipc_refresh_overview(&mut self) {
        ...
        let is_open = self.ymir.layout.is_overview_open();
    ...
    pub fn ipc_refresh_casts(&mut self) {
        ...
        #[cfg(feature = "xdp-gnome-screencast")]
        {
            ...
            for pending in &self.ymir.casting.pending_dynamic_casts {
                ...
                if !state.casts.contains_key(&stream_id) {
                    let cast = ymir_ipc::Cast {
                        session_id: pending.session_id.get(),
                        stream_id,
                        kind: ymir_ipc::CastKind::PipeWire,
                        target: ymir_ipc::CastTarget::Nothing {},
            ...
            // Check active casts.
            for cast in &self.ymir.casting.casts {
        ...
        self.ymir.screencopy_state.clear_expired_casts();

        for queue in self.ymir.screencopy_state.queues() {
            if let Some(cast_info) = queue.cast() {
                ...
                if state.casts.get(&stream_id).is_none_or(|existing| {
                    ...
                    match &existing.target {
                        ymir_ipc::CastTarget::Output { name } => *name != cast_info.output_name,
                        _ => true,
                    }
                }) {
                    let cast = ymir_ipc::Cast {
                        ...
                        stream_id,
                        kind: ymir_ipc::CastKind::WlrScreencopy,
                        target: ymir_ipc::CastTarget::Output {
