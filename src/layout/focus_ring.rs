...
use ymir_config::{CornerRadius, Gradient, GradientRelativeTo};
...
impl FocusRing {
    pub fn new(config: ymir_config::FocusRing) -> Self {
        Self {
    ...
    pub fn update_config(&mut self, config: ymir_config::FocusRing) {
        self.config = config;
    }
    ...
    pub fn config(&self) -> &ymir_config::FocusRing {
        &self.config
    }
}
