...
impl InsertHintElement {
    pub fn new(config: ymir_config::InsertHint) -> Self {
        Self {
            inner: FocusRing::new(ymir_config::FocusRing {
                off: config.off,
                width: 0.,
    ...
    pub fn update_config(&mut self, config: ymir_config::InsertHint) {
        self.inner.update_config(ymir_config::FocusRing {
            off: config.off,
            width: 0.,
            active_color: config.color,
            inactive_color: config.color,
