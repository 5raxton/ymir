...
use ymir_config::{CornerRadius, LayoutPart};
...
impl WorkspaceSwitchGesture {
    ...
    fn animate_from(&mut self, from: f64, clock: Clock, config: ymir_config::Animation) {
...
impl<W: LayoutElement> Monitor<W> {
    ...
    pub fn activate_workspace_with_anim_config(
        &mut self,
        idx: usize,
        config: Option<ymir_config::Animation>,
    ) {
    ...
    pub fn advance_animations(&mut self) {
        match &mut self.workspace_switch {
            ...
            Some(WorkspaceSwitch::Gesture(gesture)) => {
                ...
                //
                // Note: we intentionally do NOT clear dnd_nonzero_start_time here. Whether the
                // pointer is inside or outside the DnD scrolling zone is only known when a motion
                // event arrives (in dnd_scroll_gesture_scroll), which already handles clearing it
                // when delta == 0. Clearing it on every frame would re-arm the start delay
                // continuously while dragging across an edge.
    ...
    pub fn update_layout_config(&mut self, layout_config: Option<ymir_config::LayoutPart>) -> bool {
    ...
    pub fn render_insert_hint_between_workspaces<R: YmirRenderer>(
        &self,
        renderer: &mut R,
    ...
    pub fn render_workspaces<R: YmirRenderer>(
        &self,
        mut ctx: RenderCtx<R>,
        focus_ring: bool,
        push: &mut dyn FnMut(MonitorRenderElement<R>),
    ...
    pub fn render_workspace_shadows<R: YmirRenderer>(
        &self,
        renderer: &mut R,
        push: &mut dyn FnMut(MonitorRenderElement<R>),
    ) {
    ...
    pub fn layout_config(&self) -> Option<&ymir_config::LayoutPart> {
