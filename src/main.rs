...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if cli.session {
        ...
        env::set_var("XDG_CURRENT_DESKTOP", "ymir");
    ...
    ymir::utils::signals::block_early().unwrap();

    // Avoid starting Tracy for the `ymir msg` code path since starting/stopping Tracy is a bit
    // slow.
    tracy_client::Client::start();
    ...
    *CHILD_ENV.write().unwrap() = mem::take(&mut config.environment);
    ...
    ymir::utils::signals::listen(&event_loop.handle());
    ...
    let socket_name = state.ymir.socket_name.as_deref().unwrap();
    ...
    // Set YMIR_SOCKET for children.
    if let Some(ipc) = &state.ymir.ipc_server {
    ...
    if let Some(satellite) = &state.ymir.satellite {
    ...
    if cli.session {
        ...
        #[cfg(feature = "dbus")]
        if !state.ymir.config.borrow().input.disable_power_key_handling {
            if let Err(err) = state.ymir.inhibit_power_key() {
    ...
    #[cfg(feature = "dbus")]
    if cli.session {
        state.ymir.a11y.start();
    }

    if env::var_os("YMIR_DISABLE_SYSTEM_MANAGER_NOTIFY").is_none_or(|x| x != "1") {
    ...
    if config_errored {
        state.ymir.config_error_notification.show();
        state.ipc_config_loaded(true);
    } else if let Some(path) = config_created_at {
        state.ymir.config_error_notification.show_created(path);
    }
...
fn env_config_path() -> Option<PathBuf> {
    env::var_os("YMIR_CONFIG")
        .filter(|x| !x.is_empty())
        .map(PathBuf::from)
}

fn default_config_path() -> Option<PathBuf> {
    let Some(dirs) = ProjectDirs::from("", "", "ymir") else {
...
fn system_config_path() -> PathBuf {
    PathBuf::from("/etc/ymir/init.lua")
}
