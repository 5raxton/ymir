...
//!
//! This is how we map the protocol concepts to the ymir concepts:
//!
...
pub fn refresh(state: &mut State) {
    ...
    let protocol_state = &mut state.ymir.ext_workspace_state;
    ...
    for (mon, _, ws) in state.ymir.layout.workspaces() {
    ...
    protocol_state.workspace_groups.retain(|output, data| {
        if state.ymir.sorted_outputs.contains(output) {
            return true;
        }
    ...
    for (mon, ws_idx, ws) in state.ymir.layout.workspaces() {
    ...
    for output in &state.ymir.sorted_outputs {
