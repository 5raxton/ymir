...
use ymir_config::{FloatOrInt, OutputName, Vrr};
use ymir_ipc::Transform;
...
pub struct OutputManagementManagerState {
    ...
    current_state: HashMap<OutputId, ymir_ipc::Output>,
    current_config: ymir_config::Outputs,
}
...
enum OutputConfigurationState {
    Ongoing(HashMap<OutputId, ymir_config::Output>),
    Finished,
}
...
impl OutputManagementManagerState {
    ...
    pub fn on_config_changed(&mut self, new_config: ymir_config::Outputs) {
        self.current_config = new_config;
    }

    pub fn notify_changes(&mut self, new_state: HashMap<OutputId, ymir_ipc::Output>) {
                ...
                match new_config.entry(*output) {
                    ...
                    Entry::Vacant(entry) => {
                        ...
                        let mut config = g_state
                            ...
                            .find(&name)
                            .cloned()
                            .unwrap_or_else(|| ymir_config::Output {
    ...
    fn request(
        ...
        match request {
            zwlr_output_configuration_head_v1::Request::SetMode { mode } => {
                ...
                new_config.mode = Some(ymir_config::output::Mode {
                    custom: false,
                    mode: ymir_ipc::ConfiguredMode {
                        width: mode.width,
                        height: mode.height,
            ...
            zwlr_output_configuration_head_v1::Request::SetPosition { x, y } => {
                new_config.position = Some(ymir_config::Position { x, y });
            }
