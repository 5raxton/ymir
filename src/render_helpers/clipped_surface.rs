...
use super::renderer::{AsGlesFrame as _, YmirRenderer};
use super::shaders::{mat3_uniform, Shaders};
use crate::backend::tty::{TtyFrame, TtyRenderer, TtyRendererError};

#[derive(Debug)]
pub struct ClippedSurfaceRenderElement<R: YmirRenderer> {
...
impl<R: YmirRenderer> ClippedSurfaceRenderElement<R> {
    pub fn new(
    ...
    fn compute_uniforms(&self) -> Vec<Uniform<'static>> {
        ...
        vec![
            Uniform::new("ymir_scale", self.scale),
...
impl<R: YmirRenderer> Element for ClippedSurfaceRenderElement<R> {
    fn id(&self) -> &Id {
        self.inner.id()
    }
