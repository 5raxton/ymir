...
pub trait YmirRenderer:
    ImportAll
    + ImportMem
    + ExportMem
    + Bind<Dmabuf>
    + Offscreen<GlesTexture>
    + Renderer<TextureId = Self::YmirTextureId, Error = Self::YmirError>
    + AsGlesRenderer
{
    // Associated types to work around the instability of associated type bounds.
    type YmirTextureId: Texture + Clone + Send + 'static;
    type YmirError: std::error::Error
        + Send
        + Sync
...
{
    type YmirTextureId = R::TextureId;
    type YmirError = R::Error;
}
