...
use crate::dbus::mutter_screen_cast::{self, CursorMode, ScreenCastToYmir, StreamTargetId};
use crate::ymir::{CastTarget, Ymir, OutputRenderElements, PointerRenderElements, State};
...
mod pw_utils;
use pw_utils::{Cast, CastSizeChange, CursorData, PipeWire, PwToYmir};
...
pub struct Screencasting {
    ...
    pub pw_to_ymir: calloop::channel::Sender<PwToYmir>,
...
impl Screencasting {
    pub fn new(event_loop: &LoopHandle<'static, State>) -> Self {
        let pw_to_ymir = {
            let (pw_to_ymir, from_pipewire) = calloop::channel::channel();
            event_loop
...
impl State {
    fn prepare_pw_cast(&mut self) -> anyhow::Result<(GbmDevice<DrmDeviceFd>, FormatSet)> {
        ...
        if self.ymir.casting.pipewire.is_none() {
            let pw = PipeWire::new(
                self.ymir.event_loop.clone(),
                self.ymir.casting.pw_to_ymir.clone(),
            )
            .context("error initializing PipeWire")?;
            self.ymir.casting.pipewire = Some(pw);
        }
    ...
    pub fn on_pw_msg(&mut self, msg: PwToYmir) {
        match msg {
            PwToYmir::StopCast { session_id } => self.ymir.stop_cast(session_id),
            PwToYmir::Redraw { stream_id } => self.redraw_cast(stream_id),
            PwToYmir::FatalError => {
                warn!("stopping PipeWire due to fatal error");
                let casting = &mut self.ymir.casting;
                if let Some(pw) = casting.pipewire.take() {
                    ...
                    for id in ids {
                        self.ymir.stop_cast(id);
                    }
                    self.ymir.event_loop.remove(pw.token);
                }
            }
        }
    }
    ...
    fn redraw_cast(&mut self, stream_id: CastStreamId) {
        ...
        let casts = &mut self.ymir.casting.casts;
        ...
        let mut casts = mem::take(&mut self.ymir.casting.casts);
        let cast = &mut casts[idx];
        let mut stop = false;
        ...
        loop {
            ...
            let Some(output) = self.ymir.casting.mapped_cast_output.get(&mapped.window) else {
                break;
            };
            ...
            self.backend.with_primary_renderer(|renderer| {
                ...
                if self.ymir.pointer_visibility.is_visible() {
                    if let Some((pointer_pos, win_pos)) =
                        self.ymir.pointer_pos_for_window_cast(mapped)
                    {
        ...
        self.ymir.casting.casts = casts;

        if stop {
            self.ymir.stop_cast(session_id);
        }
    }
    ...
    pub fn set_dynamic_cast_target(&mut self, target: CastTarget) {
        ...
        match &target {
            ...
            CastTarget::Window { id } => {
                ...
                if let Some((_, mapped)) = windows.find(|(_, mapped)| mapped.id().get() == *id) {
                    if let Some(output) = self.ymir.casting.mapped_cast_output.get(&mapped.window) {
                        refresh = Some(output.current_mode().unwrap().refresh as u32);
                    }
                }
            }
        ...
        for cast in &mut self.ymir.casting.casts {
    ...
    fn start_pending_dynamic_casts(&mut self, target: &CastTarget) {
        let pending = &self.ymir.casting.pending_dynamic_casts;
        if pending.is_empty() {
            return;
        }
        ...
        let (size, refresh) = match target {
            ...
            CastTarget::Window { id } => {
                let Some((size, refresh)) = self.ymir.cast_params_for_window(*id) else {
                    return;
                };
                (size, refresh)
            }
        ...
        let (gbm, render_formats) = match self.prepare_pw_cast() {
            ...
            Err(err) => {
                ...
                let mut ids = HashSet::new();
                for pending in self.ymir.casting.pending_dynamic_casts.drain(..) {
        ...
        let pw = self.ymir.casting.pipewire.as_ref().unwrap();
        ...
        for pending in self.ymir.casting.pending_dynamic_casts.drain(..) {
            ...
            match res {
                Ok(mut cast) => {
                    ...
                    self.ymir.casting.casts.push(cast);
                }
                Err(err) => {
    ...
    pub fn on_screen_cast_msg(&mut self, msg: ScreenCastToYmir) {
        match msg {
            ScreenCastToYmir::StartCast {
                session_id,
                stream_id,
                target,
                cursor_mode,
                ...
                let (target, size, refresh, alpha) = match target {
                    StreamTargetId::Output { name } => {
                        let global_space = &self.ymir.global_space;
                    ...
                    StreamTargetId::Window { id }
                        if id == self.ymir.casting.dynamic_cast_id_for_portal.get() =>
                    {
                        debug!("delaying dynamic cast until target is set");
                        self.ymir.casting.pending_dynamic_casts.push(PendingCast {
                            session_id,
                            stream_id,
                            cursor_mode,
                            signal_ctx,
            ...
            ScreenCastToYmir::StopCast { session_id } => self.ymir.stop_cast(session_id),
        }
    }
}
...
impl Ymir {
    ...
    pub fn render_for_screen_cast(
        ...
        let _span = tracy_client::span!("Ymir::render_for_screen_cast");
    ...
    pub fn render_windows_for_screen_cast(
        ...
        let _span = tracy_client::span!("Ymir::render_windows_for_screen_cast");
    ...
    pub fn stop_cast(&mut self, session_id: CastSessionId) {
        let _span = tracy_client::span!("Ymir::stop_cast");
    ...
    pub fn stop_casts_for_target(&mut self, target: CastTarget) {
        let _span = tracy_client::span!("Ymir::stop_casts_for_target");
