...
use crate::ymir::{CastTarget, State};
...
pub struct PipeWire {
    ...
    to_ymir: calloop::channel::Sender<PwToYmir>,
}
...
impl PipeWire {
        ...
        let listener = core
            ...
            .error(move |id, seq, res, message| {
                ...
                if id == PW_ID_CORE && res == -32 {
                    if let Err(err) = to_ymir_.send(PwToYmir::FatalError) {
                        warn!("error sending FatalError to ymir: {err:?}");
                    }
                }
            })
            .register();
    ...
    pub fn start_cast(
        ...
        let to_ymir_ = self.to_ymir.clone();
        let stop_cast = move || {
            if let Err(err) = to_ymir_.send(PwToYmir::StopCast { session_id }) {
                warn!(%session_id, "error sending StopCast to ymir: {err:?}");
            }
        };
        let to_ymir_ = self.to_ymir.clone();
        let redraw = move || {
            if let Err(err) = to_ymir_.send(PwToYmir::Redraw { stream_id }) {
                warn!(%stream_id, "error sending Redraw to ymir: {err:?}");
            }
        };
