...
use ymir_config::animations::{Curve, EasingParams, Kind};
...
fn format_tiles(ymir: &Ymir) -> String {
    let mut buf = String::new();
    let ws = ymir.layout.active_workspace().unwrap();
...
fn set_time(ymir: &mut Ymir, time: Duration) {
    ...
    let now = ymir.clock.now();
    ymir.clock.set_unadjusted(now);
    let _ = ymir.clock.now();
    ymir.clock.set_unadjusted(Duration::ZERO);
    ymir.clock.set_rate(1.0);
    let _ = ymir.clock.now();

    // Now, set the desired time at rate 1.0.
    ymir.clock.set_unadjusted(time);
    let _ = ymir.clock.now();

    // Freeze the clock so that clear() inside the ymir loop callback followed by some get()
    // doesn't replace it with the monotonic time.
    ymir.clock.set_rate(0.0);
}
...
fn set_up() -> Fixture {
    ...
    f.ymir_state().backend.headless().add_renderer().unwrap();
...
fn set_up_two_in_column() -> (Fixture, ClientId, WlSurface, WlSurface) {
    ...
    f.ymir().layout.focus_left();
    f.ymir().layout.consume_into_column();
    f.double_roundtrip(id);
    ...
    set_time(f.ymir(), Duration::ZERO);
    f.ymir_complete_animations();
...
fn egl_height_resize_animates_next_y() {
    ...
    assert_snapshot!(format_tiles(f.ymir()), @r"
    100 × 100 at x:  0 y:  0
    200 × 200 at x:  0 y:100
    ");

    // Advance the time halfway.
    set_time(f.ymir(), Duration::from_millis(500));
    f.ymir().advance_animations();
    ...
    set_time(f.ymir(), Duration::from_millis(1000));
    f.ymir().advance_animations();
...
fn egl_clientside_height_change_doesnt_animate() {
    ...
    // The top window shrinks by itself, without a ymir-issued resize.
    let window = f.client(id).window(&surface1);
    window.set_size(100, 50);
    window.commit();
