...
fn unfocus_preserves_current_size() {
    ...
    f.ymir().layout.toggle_window_floating(None);
    f.roundtrip(id);
...
fn resize_to_different_size() {
    ...
    f.ymir().layout.toggle_window_floating(None);
    f.ymir().layout.set_column_width(SizeChange::SetFixed(500));
    f.double_roundtrip(id);
...
fn resize_to_same_size() {
    ...
    f.ymir().layout.set_column_width(SizeChange::SetFixed(200));
    ...
    // current server state, and the last size ymir requested of the window was 100×100 (even if
...
fn moving_across_workspaces_doesnt_cancel_resize() {
    ...
    f.ymir().layout.move_to_workspace_down(true);
...
fn interactive_move_unfullscreen_to_floating_restores_size() {
    ...
    let ymir = f.ymir();
    let mapped = ymir.layout.windows().next().unwrap().1;
    let window = mapped.window.clone();
    ymir.layout.set_fullscreen(&window, true);
    f.double_roundtrip(id);
    ...
    let output = f.ymir_output(1);
    let ymir = f.ymir();
    let mapped = ymir.layout.windows().next().unwrap().1;
    let window = mapped.window.clone();
    ymir.layout
        .interactive_move_begin(window.clone(), &output, Point::default());
    ymir.layout.interactive_move_update(
        &window,
        Point::from((1000., 0.)),
        output,
        Point::default(),
...
fn interactive_move_unmaximize_to_floating_restores_size() {
    ...
    let mapped = ymir.layout.windows().next().unwrap().1;
    let window = mapped.window.clone();
    ymir.layout.set_maximized(&window, true);
    f.double_roundtrip(id);
...
fn resize_during_interactive_move_propagates_to_floating() {
    ...
    f.ymir().layout.interactive_move_end(&window_id);
    f.double_roundtrip(id);
...
fn resize_in_steps() {
    ...
    f.ymir().layout.set_column_width(SizeChange::SetFixed(600));
...
fn interactive_move_restores_floating_size_when_set_to_floating() {
    ...
    // Change size while we're floating and commit to make ymir remember it.
    let window = f.client(id).window(&surface);
    window.set_size(200, 200);
    window.ack_last_and_commit();
    f.double_roundtrip(id);
...
fn unfullscreen_to_same_size_floating() {
    ...
    // Change size to the same as fullscreen, make ymir remember it.
    let window = f.client(id).window(&surface);
    window.set_size(1920, 1080);
    window.ack_last_and_commit();
    f.double_roundtrip(id);
...
fn unmaximize_to_same_size_floating() {
    ...
    // Change size to the same as maximized, make ymir remember it.
    let window = f.client(id).window(&surface);
    window.set_size(1920, 1080);
    window.ack_last_and_commit();
    f.double_roundtrip(id);
...
fn unfullscreen_to_same_size_windowed_fullscreen_floating() {
    ...
    let mapped = f.ymir().layout.windows().next().unwrap().1;
    let window_id = mapped.window.clone();
    ...
    f.ymir().layout.toggle_windowed_fullscreen(&window_id);
    f.double_roundtrip(id);
