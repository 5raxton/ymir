...
fn set_fullscreen_on_removed_output_does_not_panic() {
    ...
    // Remove the output on the ymir side. Its wl_output global is disabled but not yet
    // destroyed, so the client's wl_output resource is still valid and usable.
    let output = f.ymir_output(2);
    f.ymir().remove_output(&output);

    // Request fullscreen on the now-removed wl_output. ymir must not panic.
    let window = f.client(id).window(&surface);
