...
struct Thumbnail {
    ...
    clock: Clock,
    config: ymir_config::MruPreviews,
...
impl Thumbnail {
    fn from_mapped(mapped: &Mapped, clock: Clock, config: ymir_config::MruPreviews) -> Self {
        let app_id = with_toplevel_role(mapped.toplevel(), |role| role.app_id.clone());

        let background = FocusRing::new(ymir_config::FocusRing {
            off: false,
            width: 0.,
            active_gradient: None,
            ..Default::default()
        });
        let border = FocusRing::new(ymir_config::FocusRing {
            off: false,
            active_gradient: None,
            ..Default::default()
        });
    ...
    fn animate_move_from_with_config(&mut self, from: f64, config: ymir_config::Animation) {
    ...
    fn animate_open_with_config(&mut self, config: ymir_config::Animation) {
    ...
    fn render<R: YmirRenderer>(
        &self,
        mut ctx: RenderCtx<R>,
        config: &ymir_config::RecentWindows,
        mapped: &Mapped,
...
impl WindowMru {
    pub fn new(ymir: &Ymir) -> Self {
        let Some(output) = ymir.layout.active_output() else {
            return Self {
                thumbnails: Vec::new(),
                current_id: None,
                scope: MruScope::All,
        ...
        let config = ymir.config.borrow().recent_windows.previews;
        let mut thumbnails = Vec::new();
        for (mon, ws_idx, ws) in ymir.layout.workspaces() {
            ...
            for mapped in ws.windows() {
                let mut thumbnail = Thumbnail::from_mapped(mapped, ymir.clock.clone(), config);
...
impl WindowMruUi {
    ...
    pub fn render_output<R: YmirRenderer>(
        &self,
        ymir: &Ymir,
        output: &Output,
        mut ctx: RenderCtx<R>,
        ...
        if *output == inner.output && alpha < 1. {
            ...
            inner.render(ymir, ctx.r(), &mut |elem| elems.push(elem));
        ...
        if *output == inner.output && alpha == 1. {
            inner.render(ymir, ctx, &mut |elem| push(elem));
        }
...
impl Inner {
    ...
    fn render<R: YmirRenderer>(
        ...
        for (thumbnail, geo) in self.thumbnails_in_view_render() {
            let id = thumbnail.id;
            let Some((_, mapped)) = ymir.layout.windows().find(|(_, m)| m.id() == id) else {
                error!("window in the MRU must be present in the layout");
                continue;
            };
