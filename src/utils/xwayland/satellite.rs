...
pub fn setup(state: &mut State) {
    if state.ymir.satellite.is_some() {
        return;
    }

    let config = state.ymir.config.borrow();
    ...
    let event_loop = &state.ymir.event_loop;
    ...
    state.ymir.satellite = Some(Satellite {
        x11,
        abstract_token: None,
        unix_token: None,
        to_main,
...
fn setup_watch(state: &mut State) {
    let Some(satellite) = state.ymir.satellite.as_mut() else {
        return;
    };
    ...
    if let Some(fd) = &satellite.x11.abstract_fd {
        ...
        let token = event_loop
            .insert_source(source, move |_, _, state| {
                if let Some(satellite) = &mut state.ymir.satellite {
                    // Remove the other source.
                    if let Some(token) = satellite.unix_token.take() {
                        state.ymir.event_loop.remove(token);
                    }
                    // Clear this source.
                    satellite.abstract_token = None;

                    debug!("connection to X11 abstract socket; spawning xwayland-satellite");
                    let path = state.ymir.config.borrow().xwayland_satellite.path.clone();
...
fn spawn(path: String, xwl: &Satellite) {
    ...
    // We don't want it spamming the ymir output.
    process
        .env_remove("RUST_BACKTRACE")
        .env_remove("RUST_LIB_BACKTRACE");
    process
