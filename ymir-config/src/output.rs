...
use knuffel::Decode;
use ymir_ipc::{ConfiguredMode, HSyncPolarity, Transform, VSyncPolarity};
...
pub struct Modeline {
    ...
    pub hsync_polarity: ymir_ipc::HSyncPolarity,
    /// Vertical sync polarity: "+vsync" or "-vsync".
    pub vsync_polarity: ymir_ipc::VSyncPolarity,
}
...
pub struct MaxBpc(pub ymir_ipc::MaxBpc);
...
impl OutputName {
    pub fn from_ipc_output(output: &ymir_ipc::Output) -> Self {
        Self {
    ...
    pub fn matches(&self, target: &str) -> bool {
        ...
        //
        // This is used by ymir msg output.
        if self.make.is_none() && self.model.is_none() && self.serial.is_none() {
            return false;
        }
    ...
    fn raw_decode(
        ...
        match &**value {
            knuffel::ast::Literal::Int(ref val) => match u8::try_from(val) {
                Ok(v) => ymir_ipc::MaxBpc::try_from(v)
                    .map(MaxBpc)
