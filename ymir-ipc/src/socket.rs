//! Helper for blocking communication over the ymir socket.
...
/// Name of the environment variable containing the ymir IPC socket path.
pub const SOCKET_PATH_ENV: &str = "YMIR_SOCKET";

/// Helper for blocking communication over the ymir socket.
///
/// This struct is used to communicate with the ymir IPC server. It handles the socket connection
...
impl Socket {
    /// Connects to the default ymir IPC socket.
    ///
    ...
    pub fn connect() -> io::Result<Self> {
        let socket_path = env::var_os(SOCKET_PATH_ENV).ok_or_else(|| {
            io::Error::new(
                ...
                format!("{SOCKET_PATH_ENV} is not set, are you running this within ymir?"),
            )
        })?;
        Self::connect_to(socket_path)
    }

    /// Connects to the ymir IPC socket at the given path.
    ...
    /// Sends a request to ymir and returns the response.
    ///
    /// Return values:
    ///
    /// * `Ok(Ok(response))`: successful [`Response`](crate::Response) from ymir
    /// * `Ok(Err(message))`: error message from ymir
    /// * `Err(error)`: error communicating with ymir
